...
export default function GiveawaysBrowsePage() {
  ...
  const [postType, setPostType] =
    useState<ToolbarPost | null>("Donation and Giveaway");
  ...
  const filtered = useMemo(() => {
    ...
    const withIndex = giveaways.map((it, i) => ({ it, i }));
    ...
    return withIndex
      .filter(({ it }) => {
        ...
        if (postType && postType !== "All") {
          ...
          if (rawType) {
            const current = asPostOpt(String(rawType));
