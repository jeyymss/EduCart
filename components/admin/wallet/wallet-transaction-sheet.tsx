...
export default function WalletTransactionSheet({
  ...
  const isCreditPurchase = tx.type === "Credit Purchase";
  ...
  // Credit Purchase specific data
  const referenceCode = tx.reference_code ?? "—";
  const creditAmount = tx.amount ?? 0;
  const creditStatus = tx.status ?? "—";

  // Calculate credits based on payment amount
  const getCreditsFromAmount = (amount: number): number => {
    if (amount === 85) return 20;
    if (amount === 45) return 10;
    if (amount === 5) return 1;
    return 0; // Unknown package
  };

  const creditsEarned = getCreditsFromAmount(creditAmount);
      ...
      >
        <SheetHeader className="mb-6">
          <SheetTitle className="text-xl font-semibold">
            {isCreditPurchase ? "Credit Purchase Details" : "Transaction Details"}
          </SheetTitle>
        </SheetHeader>
        ...
        {isCreditPurchase ? (
          ...
          <>
            ...
            <div className="rounded-xl border bg-white p-5 mb-4 space-y-4">
              {/* Credits Earned */}
              <div className="flex items-center justify-between p-3 bg-gradient-to-r from-[#E59E2C]/10 to-[#E59E2C]/5 rounded-lg">
                <div className="flex items-center gap-2">
                  <Wallet className="h-5 w-5 text-[#E59E2C]" />
                  <p className="text-gray-700 font-medium">Credits Earned</p>
                </div>
                <p className="text-3xl font-bold text-[#E59E2C]">
                  {creditsEarned}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-xl font-semibold text-gray-800">
                  ₱{creditAmount.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </p>
              </div>
            ...
            <div className="p-4 rounded-xl border bg-white flex items-center justify-between">
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-semibold ${
                  creditStatus === "Completed" || creditStatus === "Paid"
                    ? "bg-green-100 text-green-700"
                    : creditStatus === "Cancelled" || creditStatus === "Failed"
